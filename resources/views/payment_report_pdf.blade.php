<?php date_default_timezone_set('asia/dhaka'); ?>
    <?php error_reporting(0);?>
    <!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Invoice - #123</title>

    <style type="text/css">
        @page {
            margin: 0px;
        }

        body {
            margin: 0px;
        }

        * {
            font-family: Verdana, Arial, sans-serif;
        }

        a {
            color: #fff;
            text-decoration: none;
        }

        table {
            font-size: x-small;
        }

        tfoot tr td {
            font-weight: bold;
            font-size: x-small;
        }

        .invoice table {
            margin: 15px;
        }

        .invoice h3 {
            margin-left: 15px;
        }

        .information {
            background-color: #60A7A6;
            color: #FFF;
        }

        .information .logo {
            margin: 5px;
        }

        .information table {
            padding: 10px;
        }
    </style>

</head>
<body>

<div class="information">
    <table width="100%">
        <tr>
            <td align="left" style="width: 40%;">
                <img src="data:image/jpeg;base64," alt="Logo" width="64" class="logo" style="text-align:center; width:180px; height:50px;"/>
            </td>
            <td>
                <h2> Visa Automation System </h2><br>
            </td>
        </tr>
    </table>
</div>


<br/>

<div class="invoice">
    <table border="1" style="table-layout: fixed; width:95%;">
        <tr>
            <td colspan="10" style="background:#083078d4; font-weight:bold; color:wheat; text-align:center;">
                @if($payment_status==0)
                    Pending Payment History | Date: <?php echo $start_date;?> - <?php echo $end_date;?>
                @endif
                    @if($payment_status==1)
                        Complete Payment History | Date: <?php echo $start_date;?> - <?php echo $end_date;?>
                    @endif
            </td>
        </tr>
        <tr>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Sl No</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Student Name</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Student ID</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Cost</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Pay</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Due Amount</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Discount</td>
        </tr>
        @foreach($payment_info as $key=>$row)
            <tr>
                <td style="width:5%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;"> {{++$key}} </td>
                <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->student_name}}</td>
                <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->student_id}}</td>
                <td style="width:15%;text-align:left; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @php
                        $total_amount=DB::table('tbl_student_payment')
                                        ->select('total_amount')
                                        ->where('student_id', '=', $row->student_id)
                                        ->first();
                    @endphp
                    {{($total_amount->total_amount)-($row->discount_amount)}}
                </td>
                <td style="width:15%;text-align:left; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->pay_amount}}</td>
                <td style="width:15%;text-align:left; font-size:10px; word-break:break-all; word-wrap:break-word;">{{($total_amount->total_amount)-($row->pay_amount)-($row->discount_amount)}}</td>
                <td style="width:15%;text-align:left; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->discount_amount}}</td>
            </tr>
        @endforeach
        <tr>
            <td colspan="3" style="text-align:right;">Total:</td>
            <td style="text-align:left;">{{($total_cost->total_amount)-($total_discount->discount_amount)}} BDT</td>
            <td style="text-align:left;">{{$total_pay->pay_amount}} BDT</td>
            <td style="text-align:left;">{{($total_cost->total_amount)-($total_pay->pay_amount)}} BDT</td>
            <td style="text-align:left;">{{$total_discount->discount_amount}} BDT</td>
            <td></td>
        </tr>
    </table>

</div>

<div class="information" style="position:absolute; width:100%; bottom: 0;">
    <table width="100%">
        <tr>
            <td align="left" style="width: 50%;">
                &copy; Visa BD - All rights reserved.
            </td>
            <td align="right" style="width: 50%;">
                Printing Time: <?php echo date("Y-m-d H:i:s A");?>
            </td>
        </tr>

    </table>
</div>
</body>
</html>




