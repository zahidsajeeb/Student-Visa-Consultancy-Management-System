<?php date_default_timezone_set('asia/dhaka'); ?>
    <!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Invoice - #123</title>

    <style type="text/css">
        @page {
            margin: 0px;
        }

        body {
            margin: 0px;
        }

        * {
            font-family: Verdana, Arial, sans-serif;
        }

        a {
            color: #fff;
            text-decoration: none;
        }

        table {
            font-size: x-small;
        }

        tfoot tr td {
            font-weight: bold;
            font-size: x-small;
        }

        .invoice table {
            margin: 15px;
        }

        .invoice h3 {
            margin-left: 15px;
        }

        .information {
            background-color: #60A7A6;
            color: #FFF;
        }

        .information .logo {
            margin: 5px;
        }

        .information table {
            padding: 10px;
        }
    </style>

</head>
<body>

<div class="information">
    <table width="100%">
        <tr>
            <td align="left" style="width: 40%;">
                <img src="data:image/jpeg;base64," alt="Logo" width="64" class="logo" style="text-align:center; width:180px; height:50px;"/>
            </td>
            <td>
                <h2> Visa Automation System </h2><br>
            </td>
        </tr>
    </table>
</div>


<br/>

<div class="invoice">
        <table border="1" style="width:95%;">
            <tr>
                <td colspan="4" style="background:#083078d4;color:wheat; font-weight:bold; text-align:center;"> Student Information</td>
            </tr>
            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Student Name</td>
                <td> {{$student_info->student_name}} </td>
                <td style="background:#f7f7f7; font-weight:bold;"> Student ID</td>
                <td> {{$student_info->student_id}} </td>
            </tr>
            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Counseling Start</td>
                <td> {{$student_info->date}} </td>
                <td style="background:#f7f7f7; font-weight:bold;"> Counselor Name</td>
                <td> {{$student_info->counselor_name}} </td>
            </tr>

            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Payable Amount</td>
                <td>
                    @if($discount_amount->discount_amount!=0)
                    {{$payable_amount->total_amount}} - {{$discount_amount->discount_amount}} =  {{($payable_amount->total_amount)-($discount_amount->discount_amount)}} BDT
                    @endif
                        @if($discount_amount->discount_amount==0)
                            {{$payable_amount->total_amount}}
                        @endif
                </td>
                <td style="background:#f7f7f7; font-weight:bold;"> Payment Status</td>
                <td>
                    @if($student_info->payment_status==0)
                        Pending
                    @endif
                        @if($student_info->payment_status==1)
                            Complete
                        @endif
                </td>
            </tr>
        </table>
    <table border="1" style="table-layout: fixed; width:95%;">
        <tr>
            <td colspan="10" style="background:#083078d4; font-weight:bold; color:wheat; text-align:center;"> Payment History</td>
        </tr>
        <tr>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Sl No</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Payment Type</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Payment Date</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Cheque No</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Bkash No</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Payment Amount</td>
        </tr>
        @foreach($payment_info as $key=>$row)
            <tr>
                <td style="width:5%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;"> {{++$key}} </td>
                <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->payment_type}}</td>
                <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->payment_date}}</td>
                <td style="width:20%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @if($row->cheque_no==0)
                        -
                    @endif
                        @if($row->pay_amount==1)
                            {{$row->cheque_no}}
                        @endif
                </td>
                <td style="width:15%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @if($row->bkash_mobile==0)
                        -
                    @endif
                    @if($row->bkash_mobile==1)
                        {{$row->bkash_mobile}}
                    @endif
                </td>
                <td style="width:20%;text-align:left; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->pay_amount}} BDT</td>
            </tr>
        @endforeach
        <tr>
            <td colspan="4"></td>
            <td style="text-align:right;">Total:</td>
            <td style="text-align:left;">{{$net_payable_amount->pay_amount}} BDT</td>
        </tr>
    </table>

</div>

<div class="information" style="position:absolute; width:100%; bottom: 0;">
    <table width="100%">
        <tr>
            <td align="left" style="width: 50%;">
                &copy; Visa BD - All rights reserved.
            </td>
            <td align="right" style="width: 50%;">
                Printing Time: <?php echo date("Y-m-d H:i:s A");?>
            </td>
        </tr>

    </table>
</div>
</body>
</html>



