<?php date_default_timezone_set('asia/dhaka'); ?>
    <!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Invoice - #123</title>

    <style type="text/css">
        @page {
            margin: 0px;
        }

        body {
            margin: 0px;
        }

        * {
            font-family: Verdana, Arial, sans-serif;
        }

        a {
            color: #fff;
            text-decoration: none;
        }

        table {
            font-size: x-small;
        }

        tfoot tr td {
            font-weight: bold;
            font-size: x-small;
        }

        .invoice table {
            margin: 15px;
        }

        .invoice h3 {
            margin-left: 15px;
        }

        .information {
            background-color: #60A7A6;
            color: #FFF;
        }

        .information .logo {
            margin: 5px;
        }

        .information table {
            padding: 10px;
        }
    </style>

</head>
<body>

<div class="information">
    <table width="100%">
        <tr>
            <td align="left" style="width: 40%;">
                <img src="data:image/jpeg;base64," alt="Logo" width="64" class="logo" style="text-align:center; width:180px; height:50px;"/>
            </td>
            <td>
                <h2> Visa Automation System </h2><br>
            </td>
        </tr>
    </table>
</div>


<br/>

<div class="invoice">
    @if(isset($name))
        <table border="1" style="width:95%;">
            <tr>
                <td colspan="4" style="background:#083078d4;color:wheat; font-weight:bold; text-align:center;"> Counselor Information</td>
            </tr>
            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Counselor Name</td>
                <td> {{$name->counselor_name}} </td>
                <td style="background:#f7f7f7; font-weight:bold;"> Counter No</td>
                <td> {{$name->counter_no}} </td>
            </tr>
            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Start Date</td>
                <td> {{$start_date}} </td>
                <td style="background:#f7f7f7; font-weight:bold;"> End Date</td>
                <td> {{$end_date}} </td>
            </tr>

            <tr>
                <td style="background:#f7f7f7; font-weight:bold;"> Total Counceling</td>
                <td> {{$total_count}} </td>
            </tr>
        </table>
    @endif
    <table border="1" style="table-layout: fixed; width:95%;">
        <tr>
            <td colspan="10" style="background:#083078d4; font-weight:bold; color:wheat; text-align:center;"> Comparison Report</td>
        </tr>
        <tr>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Sl No</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Counselor Name</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Counseling</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Amount Pending</td>
            <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Amount Complete</td>
        </tr>
        @foreach($data as $key=>$row)
            <tr>
                <td style="width:7%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;"> {{++$key}} </td>
                <td style="width:30%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->counselor_name}}</td>
                <td style="width:20%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @php
                        $total_counseling = DB::table('student_entry')
                                   ->select('*')
                                   ->where('counselor_name', $row->counter_no)
                                   ->whereBetween('date', [$start_date, $end_date])
                                   ->groupBy('student_id')
                                   ->get()
                                   ->count();
                    @endphp
                    {{$total_counseling}}
                </td>
                <td style="width:30%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @php
                        $amount_pending = DB::table('student_entry')
                                   ->select('*')
                                   ->where('counselor_name', $row->counter_no)
                                   ->whereBetween('date', [$start_date, $end_date])
                                   ->where('payment_status','0')
                                   ->get()
                                   ->count();
                    @endphp
                    {{$amount_pending}}
                </td>
                <td style="width:20%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                    @php
                        $amount_complete= DB::table('student_entry')
                                   ->select('*')
                                   ->where('counselor_name', $row->counter_no)
                                   ->whereBetween('date', [$start_date, $end_date])
                                   ->where('payment_status','1')
                                   ->get()
                                   ->count();
                    @endphp
                    {{$amount_complete}}
                </td>
            </tr>
        @endforeach
    </table>
        <table border="1" style="table-layout: fixed; width:95%;">
            <tr>
                <td colspan="10" style="background:#083078d4; font-weight:bold; color:wheat; text-align:center;"> Payment Pending Student List</td>
            </tr>
            <tr>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Sl No</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Student Name</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Student ID</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Counselor Name</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Entry Date</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Amount</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Pay</td>
                <td style="background:#f7f7f7; font-weight:bold; font-size:10px; text-align:center;">Total Pending</td>
            </tr>
            @foreach($pending_data as $key=>$row)
                <tr>
                    <td style="width:5%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;"> {{++$key}} </td>
                    <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->student_name}}</td>
                    <td style="width:15%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->student_id}}</td>
                    <td style="width:20%;text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                        @php
                            $counselor_name = DB::table('tbl_counselor')
                                       ->select('*')
                                       ->where('counter_no', $row->counselor_name)
                                       ->first();
                        @endphp
                        {{$counselor_name->counselor_name}}
                    </td>
                    <td style="width:10%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">{{$row->date}}</td>
                    <td style="width:10%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                        @php
                            $total_amount = DB::table('tbl_student_payment')
                                       ->select('total_amount')
                                       ->where('student_id', $row->student_id)
                                       ->groupBy('student_id')
                                       ->first();
                        @endphp
                        {{$total_amount->total_amount}}
                    </td>
                    <td style="width:10%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                        @php
                            $total_pay = DB::table('tbl_student_payment')
                                       ->select('*',DB::raw("SUM(pay_amount) as pay_amount"))
                                       ->where('student_id', $row->student_id)
                                       ->groupBy('student_id')
                                       ->first();
                        @endphp
                        {{$total_pay->pay_amount}}
                    </td>
                    <td style="width:10%; text-align:center; font-size:10px; word-break:break-all; word-wrap:break-word;">
                        {{($total_amount->total_amount)-($total_pay->pay_amount)}}
                    </td>
                </tr>
            @endforeach
        </table>
</div>

<div class="information" style="position:absolute; width:100%; bottom: 0;">
    <table width="100%">
        <tr>
            <td align="left" style="width: 50%;">
                &copy; Visa BD - All rights reserved.
            </td>
            <td align="right" style="width: 50%;">
                Printing Time: <?php echo date("Y-m-d H:i:s A");?>
            </td>
        </tr>

    </table>
</div>
</body>
</html>


